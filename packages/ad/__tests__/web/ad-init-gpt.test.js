...
describe("AdInit.gpt", () => {
  ...
  it("configures slots on slot init", () => {
    ...
    expect(mock.googletag.display).toHaveBeenCalledWith("mock-code-0");
  });
  ...
  it("throws if defineSlot returns null", () => {
    ...
    expect(() => mock.processGoogletagCommandQueue()).toThrowError(
      new Error(
        "Ad slot mock-code-0 /mockNetwork/mockAdUnit/mockSection could not be defined, probably it was already defined"
      )
    );
  });
});
