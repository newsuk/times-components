...
const T = ({ width }) => {
  const scale = width * SCALING_FACTOR / T_SHAPE.width;
  const transform = new Transform().translate(-T_SHAPE.x, -T_SHAPE.y);
  return (
    <Surface width={T_SHAPE.width * scale} height={T_SHAPE.height * scale}>
      <Shape
        scale={scale}
