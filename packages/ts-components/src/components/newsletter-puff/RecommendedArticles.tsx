...
import { recommendedArticles as recommendedArticlesMutation } from '@times-components/provider-queries';
  ...
  return (
    <GetNewsletter code={code} ssr={false} debounceTimeMs={0}>
      {({ isLoading, error, newsletter }: any) => {
        ...
        return (
          <Mutation
            mutation={recommendedArticlesMutation}
            onCompleted={({ recommendedArticles = {} }: any) => {
              setJustSubscribed(recommendedArticles.isSubscribed);
            }}
          >
            {(
                          ...
                          <NewsletterPuffButton
                            ...
                            onPress={() => {
                              if (!updatingSubscription) {
                                recommendedArticles({ variables: { code } });
                              }
                            }}
                          />
                        </InpSignupCTAContainer>
