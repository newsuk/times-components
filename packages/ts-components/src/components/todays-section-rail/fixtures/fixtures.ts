import { TodaysSection } from '../formatters';

export const todaysSection: TodaysSection[] = [
  {
    section: 'Sport',
    items: [
      {
        url:
          'https://www.thetimes.co.uk/article/the-entire-tour-de-france-bows-to-the-greatness-of-mark-cavendish-2zq0zgfmn',
        id: 'c286c868-d90a-11eb-8f14-0bb645f59db0',
        headline: 'The entire Tour bows to the greatness of Cavendish',

        slug:
          'the-entire-tour-de-france-bows-to-the-greatness-of-mark-cavendish',
        media: {
          crop32: {
            alt: 'some alt text',
            url:
              'https://www.thetimes.co.uk/imageserver/image/methode%2Ftimes%2Fprod%2Fweb%2Fbin%2Fb7fca746-d8ec-11eb-8f14-0bb645f59db0.jpg'
          },
          crop169: {
            alt: 'some alt text',
            url:
              'https://www.thetimes.co.uk/imageserver/image/methode%2Ftimes%2Fprod%2Fweb%2Fbin%2Fb7fca746-d8ec-11eb-8f14-0bb645f59db0.jpg'
          }
        }
      },
      {
        url:
          'https://www.thetimes.co.uk/article/jordan-pickford-repels-everything-germans-muster-ktkcvc85p',
        id: '01ee453e-d90c-11eb-8f14-0bb645f59db0',
        headline: 'Flawless Pickford repels everything Germans muster',

        slug: 'jordan-pickford-repels-everything-germans-muster',
        media: {
          crop32: {
            alt: '',
            url:
              'https://www.thetimes.co.uk/imageserver/image/methode%2Ftimes%2Fprod%2Fweb%2Fbin%2F65e48858-d913-11eb-b92f-5fe539a30c29.jpg'
          },
          crop169: {
            alt: '',
            url:
              'https://www.thetimes.co.uk/imageserver/image/methode%2Ftimes%2Fprod%2Fweb%2Fbin%2F65e48858-d913-11eb-b92f-5fe539a30c29.jpg'
          }
        }
      },
      {
        url:
          'https://www.thetimes.co.uk/article/the-tactical-masterstrokes-behind-englands-ruthless-win-s2s5j6k8l',
        id: 'a8bfaa82-d917-11eb-b92f-5fe539a30c29',
        headline: 'The tactical masterstrokes behind England’s ruthless win',

        slug: 'the-tactical-masterstrokes-behind-englands-ruthless-win',
        media: {
          crop32: {
            alt: '',
            url:
              'https://www.thetimes.co.uk/imageserver/image/methode%2Ftimes%2Fprod%2Fweb%2Fbin%2F65f0ffc0-d918-11eb-b92f-5fe539a30c29.jpg'
          },
          crop169: {
            alt: '',
            url:
              'https://www.thetimes.co.uk/imageserver/image/methode%2Ftimes%2Fprod%2Fweb%2Fbin%2F798e1dce-d918-11eb-b92f-5fe539a30c29.jpg'
          }
        }
      }
    ]
  }
];
