...
export const UpdateButtonWithDelayStory = ({ loading, display, delay, label, arrowUp }) => (
  <TCThemeProvider theme={TimesWebLightTheme}>
    <UpdateButtonWithDelay loading={loading} display={display} delay={delay} label={label} arrowUp={arrowUp} />
  </TCThemeProvider>
);

<Story name="UpdateButtonWithDelay"
  args={{ loading: false, display: true, delay: 8000, label: 'New Updates', arrowUp: true} }
>
