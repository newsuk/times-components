...
## Code Example
`<UpdateButtonWithDelay loading={loading} display={true} delay={8000} label='New updates', arrowUp={true}/>`
...
export const UpdateButtonWithDelayStory = ({ loading, display, delay, label, arrowUp, updatedTime }) => (
  <TCThemeProvider theme={TimesWebLightTheme}>
    <UpdateButtonWithDelay loading={loading} display={display} delay={delay} label={label} arrowUp={arrowUp} updatedTime={updatedTime} />
  </TCThemeProvider>
);

<Story name="UpdateButtonWithDelay"
  args={{ loading: false, display: true, delay: 8000, label: 'New Updates', arrowUp: true, updatedTime: '2023-07-13T12:00:00.000Z'} }
>
