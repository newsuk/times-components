...
## Code Example
`<SliceHeader title='Title' color='#008347' />`
...
export const HeaderStory = ({ title , color}) => (
  <NewsKitProvider theme={TimesWebLightTheme}>
    <SliceHeader {...{ title,color }} />
  </NewsKitProvider>
);

<Story name="SliceHeader" args={{ title: 'Rugby Union', color : '#008347' }}>
  {HeaderStory.bind({})}
</Story>