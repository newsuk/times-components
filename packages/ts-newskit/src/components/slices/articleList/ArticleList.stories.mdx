...
export const ArticleListStory = ({ image, color, alt, title, url, articleType, timeToRead }) => (
  <NewsKitProvider theme={TimesWebLightTheme}>
    <ArticleListItem {...{ image, color, alt, title, url, articleType, timeToRead }} />
  </NewsKitProvider>
);

<Story name="ArticleListItem" args={{ image, color, alt, title, url, articleType, timeToRead }}>
