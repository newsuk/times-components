...
module.exports = () => {
  context("DEFAULT template", () => {
    ...
    it("renders a single child element", () => {
      const tree = renderer
        .create(<Slice template={template}>{createItems(1)}</Slice>)
        .toJSON();
    ...
    it("renders two child elements", () => {
      const tree = renderer
        .create(<Slice template={template}>{createItems(2)}</Slice>)
        .toJSON();
      expect(tree).toMatchSnapshot();
    });
    ...
    it("renders three child elements", () => {
      const tree = renderer
        .create(<Slice template={template}>{createItems(3)}</Slice>)
        .toJSON();
      expect(tree).toMatchSnapshot();
    });
  });
