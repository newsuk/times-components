...
- If you receive a `":CFBundleIdentifier", Does Not Exist` error when trying to run `yarn ios`, try clearing your React Native cache with `rm -r ~/.rncache` and clearing third part libraries `rm -r <your-project>/node_modules/react-native/third_party`. This happens to do React Native cachine third party tools for previous versions of React Native.
